...
fn return_by_pointer(hlr: &mut FuncRep) {
    ...
    hlr.modify_many(|data| matches!(data, NodeData::Return {..}), |return_id, data, hlr| {
...
fn format_call_returning_struct(hlr: &mut FuncRep, og_call: ExprID) {
    ...
    hlr.insert_statement_before(og_call, MakeVarGen {
...
fn format_call_returning_pointer(hlr: &mut FuncRep, og_call_id: ExprID) {
    ...
    hlr.insert_statement_before(og_call_id, MakeVarGen {
        set: call_var.clone(),
    ...
    hlr.insert_statement_before(og_call_id, CallGen {
        info: UniqueFuncInfo {
            name: f,
            generics,
            relation,
